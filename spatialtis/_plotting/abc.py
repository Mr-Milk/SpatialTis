...
@dataclass
class PlotBase:
    """Base class for all _plotting instance

    Noticed that following arguments/attributes might not works for all the _plotting instances

    Args:
        plot: The plot instance, only works for Bokeh and Pyecharts plot.
        data: The data used for _plotting
        groupby: {groupby}
...
class PyechartsMixin(PlotBase):
    ...
    def set_up(self):
        """Handle title, display and save"""
        if self.title is not None:
            self.plot.set_global_opts(
                title_opts=opts.TitleOpts(self.title),
            )
        if self.display:
            self.plot.load_javascript()
