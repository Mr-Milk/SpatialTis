...
def dotplot(df,
    ...
    circles = [plt.Circle((j + 0.5, i + 0.5), radius=r, ) for r, j, i in zip(R.flat, X.flat, Y.flat)]
    if annotated:
    ...
    ax.set(yticks=np.arange(0.5, M + 1),
    ...
    legends = [
        Line2D((), (), color="white", marker='o', markerfacecolor="black", label=f"{int(size.max())}", markersize=1.0),
        Line2D((), (), color="white", marker='o', markerfacecolor="black", label=f"{int(size.max() / 4 * 3)}",
               markersize=0.75),
        Line2D((), (), color="white", marker='o', markerfacecolor="black", label=f"{int(size.max() / 2)}",
               markersize=0.5),
    ...
    legend = ax.legend(handles=legends, loc='upper left', bbox_to_anchor=(1, -0.1, 0, 1), title="Size", markerscale=21,
                       labelspacing=1.2)
