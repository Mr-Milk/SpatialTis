...
from bokeh.models import (ColorBar, ColumnDataSource, Legend, LegendItem,
                          LinearColorMapper)
...
from spatialtis.plotting import get_colors, get_linear_colors
...
def heatmap(
    ...
    figure_config = dict(
        title=title, tools=TOOLS, toolbar_location=None, y_axis_location="right"
    )
    if size is not None:
    ...
    if row_cluster:
        r_icd, r_dcd, r_labels = dendrogram_coord(
            df.values, metric=metric, method=method
        )
        df = df.iloc[r_labels]

    # calculate col_dendrogram
    if col_cluster:
        c_icd, c_dcd, c_labels = dendrogram_coord(
            df.T.values, metric=metric, method=method
        )
        df = df.iloc[:, c_labels]
    ...
    if isinstance(palette, str):
        if palette in default_palette:
            ...
            raise ValueError(
                f"Not a build-in palette, Options are 'Cividis', 'Gray', 'Inferno', 'Magma', 'Viridis'"
            )
    ...
    try:
        ...
        raise ValueError(
            f"Plotting heatmap only accept built-in linear palette or Sequence of user-defined colors"
        )
    ...
    data = pd.DataFrame(
        dict(col_coord=col_coord, index_coord=index_coord, values=nd.ravel(),)
    )
    ...
    row_dendro_offset = (
        row_colors_standoff + row_block_width * len(row_colors)
        if row_colors_show
        else 0
    )
    col_dendro_offset = (
        col_colors_standoff + col_block_height * len(col_colors)
        if col_colors_show
        else 0
    )
    ...
    if row_cluster & row_dendrogram:
        row_icd = (r_icd - np.min(r_icd)) / (np.max(r_icd) - np.min(r_icd)) * (
            ysmax - ysmin
        ) + 0.5
        row_dcd = (r_dcd - np.min(r_dcd)) / (
            np.max(r_dcd) - np.min(r_dcd)
        ) * dendrogram_ratio * x_len + row_dendro_offset

        p.multi_line(list(-row_dcd), list(row_icd), line_alpha=0.8, line_color="black")

    # plot col-dendrogram
    if col_cluster & col_dendrogram:
        col_icd = (c_icd - np.min(c_icd)) / (np.max(c_icd) - np.min(c_icd)) * (
            xsmax - xsmin
        ) + 0.5
        col_dcd = (
            (c_dcd - np.min(c_dcd))
            / (np.max(c_dcd) - np.min(c_dcd))
            * dendrogram_ratio
            * y_len
            + ysmax
            + 0.5
    ...
    if row_colors_show:
        ...
        def add_row_blocks(blocks, step):
            plot_blocks = ColumnDataSource(
                dict(
                    x=np.array(
                        [
                            -row_block_width / 2 - row_block_width * step
        ...
        if row_colors_legend:
            ...
            row_legend = Legend(
                title=row_colors_legend_title,
                items=row_colors_legends,
    ...
    if col_colors_show:
        if col_depth == 1:
            ...
            df_col = pd.DataFrame(
                [list(i) for i in df.columns], columns=df.columns.names
            )
        ...
        def add_col_blocks(blocks, step):
            plot_blocks = ColumnDataSource(
                dict(
                    ...
                    y=np.array(
                        [
                            ysmax + 0.5 + col_block_height / 2 + col_block_height * step
                            for i in range(0, len(blocks))
                        ]
                    )
                    + col_colors_standoff,
        ...
        if col_colors_legend:
            ...
            col_legend = Legend(
                title=col_colors_legend_title,
                items=col_colors_legends,
                location="top_left",
                label_standoff=0,
                border_line_color=None,
            )
    ...
    if save_svg:
        ...
        Figure(
            f"{pw + 200}px", f"{ph}px", SVG(heat_svg), SVG(legend_svg).move(pw, 0),
        ).save(save_svg)
        # .tile(2, 1)
