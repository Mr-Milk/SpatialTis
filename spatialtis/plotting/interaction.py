...
from milkviz import dot_heatmap, dot, anno_clustermap
from milkviz.utils import mask_triu
from scipy.stats import pearsonr

from spatialtis import get_result, Config
...
def count_size_side(rdata, type_order):
    ...
    dot_size = pairs_to_adj(pd.DataFrame(dot_size, index=[0]).T.reset_index(), type_order)
...
def cell_interaction(data: AnnData,
    ...
    if use == "heatmap":
        ...
        if not order:
            uni_types = np.unique(rdata.columns.to_frame().to_numpy()) if type_order is None else type_order
            rdata = rdata[[tuple(c) for c in combinations_with_replacement(uni_types, 2)]]
        else:
            if type_order is not None:
                rdata = rdata[[tuple(c) for c in product(type_order, repeat=2)]]
        options = dict(categorical_cbar=['Avoidance', 'Association'],
                       ...
                       vmin=-1, vmax=1,
                       **plot_options)
        return anno_clustermap(rdata, col_colors=['type1', 'type2'], row_colors=groupby,
                               **options)
    else:
        ...
        try:
            ...
            matrix = matrix.loc[dot_size.index, dot_size.columns]  # ensure the number match to data
        except Exception:
            log_print("Run spatialtis.cell_components to add "
        ...
        options = dict(size_legend_title="Sign' ROI",
        ...
        return dot_heatmap(dot_size=dot_size_data,
