...
@doc
def cell_dispersion(data: AnnData,
                    key: str = "cell_dispersion",
                    **plot_options,
                    ):
    pdata, param = get_result(data, key, params=True)
    pdata = pd.pivot(pdata, columns="cell_type", index=param['exp_obs'], values='pattern')
    pattern = {}
    for t, arr in pdata.iteritems():
        pattern[t] = {0: 0, 1: 0, 2: 0, 3: 0, **Counter(arr)}
    pdata = pd.DataFrame(pattern).T[[0, 1, 2, 3]]
    colors = np.repeat([["#FFC408", "#c54a52", "#4a89b9", "#5a539d"]], len(pdata), axis=0)
    return dot(dot_size=pdata.to_numpy(),
               dot_hue=colors,
               legend_title="ROI",
