...
ISOTOPES_NAME = ['H', 'He', 'Li', 'Be', 'B', 'C', 'N', 'O', 'F', 'Ne', 'Na', 'Mg', 'Al', 'Si', 'P', 'S', 'Cl', 'Ar', 'K', 'Ca', 'Sc', 'Ti', 'V', 'Cr', 'Mn', 'Fe', 'Co', 'Ni', 'Cu', 'Zn', 'Ga', 'Ge', 'As', 'Se', 'Br', 'Kr', 'Rb', 'Sr', 'Y', 'Zr', 'Nb', 'Mo', 'Tc', 'Ru', 'Rh', 'Pd', 'Ag', 'Cd', 'In', 'Sn', 'Sb', 'Te', 'I', 'Xe', 'Cs', 'Ba', 'La', 'Ce', 'Pr', 'Nd', 'Pm', 'Sm', 'Eu', 'Gd', 'Tb', 'Dy', 'Ho', 'Er', 'Tm', 'Yb', 'Lu', 'Hf', 'Ta', 'W', 'Re', 'Os', 'Ir', 'Pt', 'Au', 'Hg', 'Tl', 'Pb', 'Bi', 'Po', 'At', 'Rn', 'Fr', 'Ra', 'Ac', 'Th', 'Pa', 'U', 'Np', 'Pu', 'Am', 'Cm', 'Bk', 'Cf', 'Es', 'Fm', 'Md', 'No', 'Lr', 'Rf', 'Db', 'Sg', 'Bh', 'Hs', 'Mt', 'Ds', 'Rg', 'Cn', 'Nh', 'Fl', 'Mc', 'Lv', 'Ts', 'Og']
...
def get_cell_exp_single_channel(channel, cells, method='mean'):
    '''
    Parameters
    channel: Path/str, path to channel image
    cells: list, each element contains points for each cell
    method: 'mean', 'median', 'sum', how to compute expression level of single cell
    '''
    cells_density = list()
    for cell in cells:
...
def get_cell_exp_stack(stack, cells, method='mean'):
    '''
    Parameters
    channel: Path/str, path to channel image
    cells: list, each element contains points for each cell
...
class read_all_ROIs:
    ...
    def __init__(self, entry, condition_name, mask_pattern="*mask*"):
        '''
        ...
        self._tree = []
        self._obs_name = condition_name
        self._depth = len(condition_name)
    ...
    def to_annData(self):
        ...
        for i, d in enumerate(self._tree):
            ...
            if i == 0:
                X = np.array(exp, dtype=object)
                cells_X = np.array(cells, dtype=object)
                ann_obs = obs
            else:
                X = np.concatenate((X, np.array(exp, dtype=object)), axis=0)
        ...
        return ad.AnnData(X, obs=ann_obs, var=self._var, dtype=object)
        ...
        for img in Path(folder).iterdir():
            if img not in mask:
                with tifffile.TiffFile(str(img)) as tif:
                    ...
                    pattern = re.compile(r'([a-zA-Z]{1,})([0-9]{2,})')
